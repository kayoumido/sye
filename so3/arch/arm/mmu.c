...
void configure_l1pgtable(uint32_t l1pgtable, uint32_t fdt_addr) {
	...
	__pgtable[CONFIG_RAM_BASE >> L1_PAGETABLE_SHIFT] = CONFIG_RAM_BASE  | L1DESC_SECT_AP01 | L1DESC_SECT_AP2 | L1DESC_TYPE_SECT;

	/* Now, create a virtual mapping in the kernel space */
	for (i = 0; i < 6; i++)
		__pgtable[(CONFIG_KERNEL_VIRT_ADDR >> L1_PAGETABLE_SHIFT) + i] = (((CONFIG_RAM_BASE >> L1_PAGETABLE_SHIFT) + i) << L1_PAGETABLE_SHIFT) | L1DESC_SECT_AP01 | L1DESC_SECT_AP2 | L1DESC_TYPE_SECT;

...
/* Used to read from a serial (uart) console. We report only one byte when the byte is ready. */
static int fd_vuart_getc(int gfd, void *buffer, int count)
{
	/* Read one byte from the UART console */
	*((uint8_t *) buffer) = vuart_getc();

	return 1;
}

/* Send out to the serial console. */
static int fd_vuart_write(int gfd, void *buffer, int count)
{
	vuart_write((char *) buffer, count);
